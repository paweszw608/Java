...
public class Group  implements Serializable {
    ...
    /**
     * Sorting teams in teamsArrayList by their points using bubble sort algorithm
     */
    public void sortTeams() {
        int size = teamsArrayList.size();
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size - i - 1; j++) {
                if (teamsArrayList.get(j + 1).getPoints() > teamsArrayList.get(j).getPoints())
                     Collections.swap(teamsArrayList, j, j+1);
            }
        }
    }
    ...
    public String[] getStandings() {
        ...
        for (int i = 0; i < teamsArrayList.size(); i++) {
            teamsListStrings[i] = (i + 1) + ". " + teamsArrayList.get(i).getName() + " "
                    + String.valueOf(teamsArrayList.get(i).getPoints() + " pts");
        }
        return teamsListStrings;
    }
